...
module DFlipFlop(input stall, clk, input rst, input D, output reg Q);
...
module n_bit_register #(parameter n = 8) (input stall, input [n-1:0] D, input rst, input Load, input clk, output [n-1:0] Q);
...
generate
	for(i = 0; i < n; i = i+1) begin: myblockname
		...
		DFlipFlop dflpflop(stall, clk, rst, DD[i], Q[i]);	
    end
endgenerate
endmodule
...
generate
    for (i = 0; i < 32; i = i+1) begin: myblockname
       n_bit_register #(32) nb(1'b0, WriteData, rst, load[i], clk,  Q[i]);
    end
endgenerate
endmodule
